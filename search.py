...
def get_postings(word:str) -> list:
    ...
    target_path = current_dir + "/results/" + first_letter + str(num) + ".json"
    all_postings = []
    valid_path = os.path.exists(target_path)
    while valid_path:
        json_content = get_json_content(current_dir + "/results/" + first_letter + str(num) + ".json")
        if word in json_content:
            all_postings.extend(json_content[word])
        num += 1
        valid_path = os.path.exists(current_dir + "/results/" + first_letter + str(num) + ".json")
